...
//        xCoords.at<float>(i) = std::max(std::min(xCoords.at<float>(i),
//        static_cast<float>(shape.width - 1)), 0.0f); yCoords.at<float>(i) =
//        std::max(std::min(yCoords.at<float>(i), static_cast<float>(shape.height - 1)), 0.0f);
//    }
//}
...
void clip_coords(std::vector<float>& coords, const cv::Size& shape)
{
  ...
  for (int i = 0; i < coords.size(); i += 3)
  {
    ...
    coords[i + 1] =
        std::min(std::max(coords[i + 1], 0.0f), static_cast<float>(shape.height - 1)); // y
  }
}
...
// cv::Mat scale_coords(const cv::Size& img1_shape, std::vector<float> coords, const cv::Size&
// img0_shape)
std::vector<float>
