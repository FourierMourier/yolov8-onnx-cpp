...
/*
 * This interface must provide only required arguments to load any onnx model regarding specific
 * info -
 *  - i.e. modelPath will always be required, provider like "cpu" or "cuda" the same, since these
 * are parameters you need to set up `sessionOptions` or `session` objects properly, but image size
 * is not needed for pure onnx graph to be loaded so do NOT include it here
 */

class OnnxModelBase
{
public:
  OnnxModelBase(const char* modelPath, const char* logid, const OnnxProviders_t provider);
...
#endif // YOLOV8_ONNXRUNTIME_ONNX_MODEL_BASE_H